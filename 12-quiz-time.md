...
```
e ReactDOM.render method returns JSX to the Kangaroos component, and the Kangaroos component returns a virtual DOM node. React updates on the webpage only the elements specified in Kangaroos that have changed.```
...
```
In the component class' render() method; you always need it for accurate setup of that class.
